    ...
    public void run() {
        ...
        try {
            while (gotSemaforePermits < ConnectionFactory.MAX_CONNECTIONS) {
                ...
                gotSemaforePermits++;
                logger.log(Level.DEBUG, "pool service got permits {} from {}", gotSemaforePermits,
                        ConnectionFactory.MAX_CONNECTIONS);
            }
        ...
        while (activeConnections < ConnectionFactory.MAX_CONNECTIONS) {
            try {
                ...
                try {
                    ...
                    logger.log(Level.ERROR, "Thread in pool service has been interrupted! :{}",
                            ex.getMessage());
                    Thread.currentThread().interrupt();
                }
            } catch (InterruptedException e) {
