...
public class ProductDaoImpl implements ProductDao {
    ...
    private static final String SQL_ADD_NEW_PRODUCT = """
            INSERT INTO lightingshop.products (brand_id, type_id, product_name, description, price, colour, power,
             size, photo, quantity_in_stock)
            values (?,?,?,?,?,?,?,?,?,?)""";
    ...
    private static final String SQL_UPDATE_PRODUCT_DATA = """
            UPDATE lightingshop.products
            SET brand_id =?, type_id =?, product_name =?, description =?, price =?, colour =?, power =?, size =?,
             quantity_in_stock =?
            WHERE id =?
            """;
    private static final String SQL_SELECT_PRODUCT_BY_VAR_PARAM = """
            SELECT id, brand_id, type_id, product_name, description, price, colour, power, size, photo, 
            quantity_in_stock
            FROM lightingshop.products WHERE %s = ?""";
    private static final String SQL_SELECT_ALL_PRODUCTS = """
            SELECT id, brand_id, type_id, product_name, description, price, colour, power, size, photo,
             quantity_in_stock FROM lightingshop.products""";
    ...
    public Optional<Product> findEntityByOneParam(String paramName, String paramValue) throws DaoException {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                logger.log(Level.DEBUG, "product was found by param {} and param value {}", paramName,
                        paramValue);
            }
    ...
    public boolean create(Product product, byte[] photo, int quantityInSock) throws DaoException {
        ...
        try {
            ...
            try {
                ...
                logger.log(Level.ERROR, "ProductDao error while create new product closing resources. {}",
                        e.getMessage());
            }
        }
        return true;
    ...
    public Map<Product, String> findBySearchParam(int brandId, int typeId, int minPrice, int maxPrice,
                                                  ...
                                                  String searchWord, String orderQuery, int inStock)
            throws DaoException {
        ProxyConnection proxyConnection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        ...
        try {
            proxyConnection = DbConnectionPool.getInstance().takeConnection();
            preparedStatement =
                    proxyConnection.prepareStatement(String.format(SQL_SELECT_PRODUCTS_BY_PARAMS, FIND_ANY
                            + searchWord + FIND_ANY, orderQuery));
            if (brandId == 0) {
    ...
    @Override
    public int countBySearchParam(int brandId, int typeId, int minPrice, int maxPrice, int minPower, int maxPower,
                                  String searchWord, int inStock) throws DaoException {
        ProxyConnection proxyConnection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        int result = 0;
        ...
        try {
            proxyConnection = DbConnectionPool.getInstance().takeConnection();
            preparedStatement =
                    proxyConnection.prepareStatement(String.format(SQL_COUNT_PRODUCTS_BY_PARAMS, FIND_ANY
                            + searchWord + FIND_ANY));
            if (brandId == 0) {
                preparedStatement.setString(1, FIND_ANY);
            } else {
    ...
    @Override
    public Map<Product, String> findMapProductQuantityById(String paramName, String paramValue, Map<Product,
            String> productQuantity) throws DaoException {
    ...
    public boolean updatePhoto(int id, byte[] bytesPhoto) throws DaoException {
        ...
        try {
            ...
            try {
                ...
                logger.log(Level.ERROR, "ProductDao error while updatePhoto closing resources. {}",
                        e.getMessage());
            }
        }
        return result > 0;
