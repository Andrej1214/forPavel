...
public class UserDaoImpl implements UserDao {
    ...
    private static final String SQL_SELECT_USER_BY_LOGIN_AND_PASSWORD = """
            SELECT id, first_name, last_name, login, password, email, phone, access_level,
             registration_date,money_amount
    ...
    private static final String SQL_FIND_ALL_USER_ORDER_BY_PARAM = """
            SELECT id, first_name, last_name, login, password, email, phone, access_level, 
            registration_date,money_amount FROM lightingshop.users
            ORDER BY last_name
            """;
    ...
    public static final String SQL_FIND_ALL_USER_ORDER_SUM = """
            ...
            SUM(CASE WHEN orders.status ='DELIVERED' THEN products.price*order_products.quantity END) 
            AS total_ords_sum FROM lightingshop.users
    ...
    public static final String SQL_FIND_ORDER_SUM_BY_PARAM = """
             SELECT orders.id, orders.user_id, orders.address, orders.phone, orders.detail, orders.status,
              orders.date_open, orders.date_close, users.login,SUM(products.price*order_products.quantity)
               AS total FROM lightingshop.orders
            ...
            while (resultSet.next()) {
                ...
                if (optionalUser.isPresent()) {
                    userMap.put(optionalUser.get(), new double[]
                            {resultSet.getDouble("number_prods"),
                                    resultSet.getDouble("total_ords_sum")});
                }
            }
        } catch (SQLException e) {
            proxyConnection.setForChecking(true);
    ...
    @Override
    public boolean createOrderTransaction(User currentUser, Cart currentCart, Map<String, String> orderDataMap)
            throws DaoException {
        ProxyConnection proxyConnection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
...
// отключено по требованию преподавателя
            ...
            for (Map.Entry<Integer, Integer> entry : prodIdQuantity.entrySet()) {
                preparedStatement = proxyConnection.prepareStatement(SQL_FIND_PRODUCT_BY_ID,
...
// отключено по требованию преподавателя
            ...
            //return user's money
            preparedStatement =
                    proxyConnection.prepareStatement(String.format(SQL_SELECT_USER_BY_VAR_PARAM, ID),
                            ResultSet.TYPE_FORWARD_ONLY,
                            ResultSet.CONCUR_UPDATABLE);
    ...
    @Override
    public boolean findOrderMapByParam(Map<Order, String[]> orderMap, Map<String, String> incomeParam)
            throws DaoException {
        ProxyConnection proxyConnection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
