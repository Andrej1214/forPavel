...
public class UserServiceImpl implements UserService {
    ...
    /**
     * find {@link User} by one parameter (ID, EMAIL, LOGIN...)
     * if number of user have the same pair param - value, returns first found
     *
     * @param paramName
     * @param paramValue
     * @return Optional {@link User}
     ...
     */
    @Override
    public boolean updateUserMoneyAmount(int currentUserId, BigDecimal currentUserAmount, String amountToDeposit)
            throws
            ServiceException {
        UserDao userDao = UserDaoImpl.getInstance();
     ...
     * check if user has enough money to make this transaction,
     * if not - add MESSAGE ORDERING_FAIL_NOT_ENOUGH_PRODUCTS to orderDataMap, return false
     ...
     * if not - correct quantity in currentCart = quantity in stock.
     * If quantity in stock == 0< remove this product from currentCart and return false
     *
     * @param currentCart
     * @return boolean as result
     * @throws ServiceException
    ...
    private boolean isStillEnoughProductInStock(Cart currentCart) throws ServiceException {
        ...
        try {
            for (Map.Entry<Product, Integer> entry : productQuantity.entrySet()) {
                ...
                productDao.findMapProductQuantityById(ID, String.valueOf(entry.getKey().getId()),
