...
public class ParameterValidatorImpl implements ParameterValidator {
    ...
    @Override
    public boolean validateAndMarkIfProductNameCorrectAndNotExistsInDb(Map<String, String> productData)
            ...
            for (Map.Entry<String, String> entry : productData.entrySet()) {
                ...
                if (key.equals(PRODUCT_NAME)) {
                    if (!validator.isProductNameValid(entry.getValue())) {
                        ...
                        Optional<Product> optionalProduct = productDao.findEntityByOneParam(PRODUCT_NAME,
                                entry.getValue());
                        if (optionalProduct.isPresent()) {
                            Product product = optionalProduct.get();
                            if (productData.get(ID) != null &&
                                    (product.getId() != (Integer.parseInt(productData.get(ID))))) {
    ...
    public boolean validateJpg(byte[] photo, int... widthAndHeight) {
        ...
        if (photo.length > MAX_FILE_VALUE) {
            logger.log(Level.INFO,
                    "ParameterValidator validateJpg file too big, allowed length is 4194304, but {}",
                    photo.length);
            return false;
        }
    ...
    @Override
    public boolean validateAndCorrectSearchProductParam(Map<String, String> searchProductParam, Map<String,
            String> orderByParamQuery) {
        boolean isCorrect = true;
        AttributeValidator validator = AttributeValidatorImpl.getInstance();
        ...
        for (Map.Entry<String, String> entry : searchProductParam.entrySet()) {
            switch (entry.getKey()) {
                ...
                case ORDER_BY -> {
                    ...
                    if (orderBy == null || !Stream.of(orderByParamQuery)
                            .anyMatch(entry2 -> entry2.containsKey(orderBy))) {
                ...
                switch (key) {
                    ...
                    case TYPE_ID -> {
                        if (!validator.isInt5Valid(entry.getValue())) {
                            ...
                            Optional<ProductType> optionalType = productTypeDao.findEntityByOneParam(ID,
                                    entry.getValue());
