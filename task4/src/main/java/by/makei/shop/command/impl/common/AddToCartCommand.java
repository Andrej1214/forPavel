            ...
            if ((productService.findMapProductQuantityById(inputProductIdQuantity, productQuantityMapFromDB))
                ...
                if (productQuantityMapFromDB.size() != 1) {
                    logger.log(Level.WARN,
                            "AddToCartCommand error while find product in DB, quantity of product = {}",
                            productQuantityMapFromDB.size());
                ...
                if (currentQuantity > Cart.MAX_QUANTITY_OF_ONE_PRODUCT_TO_BY) {
                    logger.log(Level.INFO, "AddToCartCommand asked product quantity more than "
                            + Cart.MAX_QUANTITY_OF_ONE_PRODUCT_TO_BY);
                ...
                router.setRedirectType();
                router.setCurrentPage(GO_TO_SHOW_CART + RedirectMessage.REDIRECT_MESSAGE
                        + RedirectMessage.ADDING_TO_CART_FAIL_INCORRECT_DATA);
            }
