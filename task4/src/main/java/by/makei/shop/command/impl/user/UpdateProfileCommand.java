    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            if (parameterValidator.validateAndMarkUserData(userDataMap)) {
                ...
                optionalUser = userService.findUserByOneParam(AttributeName.LOGIN,
                        userDataMap.get(AttributeName.LOGIN));
                ...
                optionalUser = userService.findUserByOneParam(AttributeName.EMAIL,
                        userDataMap.get(AttributeName.EMAIL));
                if (optionalUser.isPresent() && (optionalUser.get().getId() != Integer.parseInt(currentUserid))) {
                    logger.log(Level.ERROR, "UpdateProfileCommand busy email");
                    isCorrect = false;
                    userDataMap.put(AttributeName.BUSY_EMAIL, AttributeName.BUSY_EMAIL);
                }
                optionalUser = userService.findUserByOneParam(AttributeName.PHONE,
                        userDataMap.get(AttributeName.PHONE));
                if (optionalUser.isPresent() && (optionalUser.get().getId() != Integer.parseInt(currentUserid))) {
                ...
                if (isCorrect) {
                    ...
                    if (optionalUser.isPresent()) {
                        ...
                        router.setCurrentPage(GO_TO_MAIN + RedirectMessage.REDIRECT_MESSAGE
                        ...
                        request.setAttribute(AttributeName.ERROR_MESSAGE,
                                "UpdateProfile command user wasn't found after update profile");
