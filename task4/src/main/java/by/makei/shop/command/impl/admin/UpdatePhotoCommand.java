...
public class UpdatePhotoCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        try (
            ...
            logger.log(Level.ERROR, "error while updatePhotoCommand is trying to get photo. {}",
                    e.getMessage());
        ...
        try {
            if (productService.updatePhoto(productDataMap, bytesPhoto)) {
                ...
                router.setCurrentPage(GO_TO_UPDATE_PRODUCT + RedirectMessage.REDIRECT_MESSAGE
                        + RedirectMessage.UPDATE_SUCCESS + RedirectMessage.REDIRECT_ID
                        + request.getParameter(AttributeName.ID));
            } else {
                logger.log(Level.INFO, "UpdatePhotoCommand incorrect photo");
                router.setRedirectType();
                router.setCurrentPage(GO_TO_UPDATE_PRODUCT + RedirectMessage.REDIRECT_MESSAGE
                        + RedirectMessage.UPDATE_FAIL + RedirectMessage.REDIRECT_ID
                        + request.getParameter(AttributeName.ID));
            }
