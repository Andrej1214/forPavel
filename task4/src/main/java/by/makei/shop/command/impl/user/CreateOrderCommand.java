    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        if (currentCart.getTotalQuantity() == 0) {
            logger.log(Level.ERROR,
                    "CreateOrderCommand empty cart. TotalCost = '{}', and userId ='{}' amount = '{}'"
        ...
        if (commonOrderPrice.compareTo(currentUser.getAmount()) > 0) {
            logger.log(Level.ERROR,
                    "CreateOrderCommand not enough money to transaction. TotalCost = '{}', and userId ='{}' amount = '{}'"
                    , commonOrderPrice.doubleValue(), currentUser.getId(), currentUser.getAmount().doubleValue());
            ...
            if (userService.createOrder(currentUser, currentCart, orderDataMap)) {
                ...
                optionalUser.ifPresentOrElse(user -> {
                            ...
                            router.setCurrentPage(GO_TO_MAIN + RedirectMessage.REDIRECT_MESSAGE
                                    + RedirectMessage.SUCCESSFULLY_ORDERED);
                        }
